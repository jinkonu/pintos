...
/* Finds and returns the starting index of the first group of CNT
   ...
   VALUE.
   If there is no suchcn  group, returns BITMAP_ERROR. */
size_t
...
// edited
struct bitmap *bitmap_expand(struct bitmap *b, size_t cnt) {
    struct bitmap *tmp = bitmap_create(bitmap_size(b) + cnt);   // augmented new bitmap
    for (int i = 0; i < bitmap_size(b); i++) {  // copying
        if (bitmap_contains(b, i, 1, true)) bitmap_mark(tmp, i);
    }

    for (int i = 0; i < bitmap_size(tmp); i++) {
        printf("0");
    }
    return tmp;
}