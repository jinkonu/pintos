...
list_splice (struct list_elem *before,
             struct list_elem *first, struct list_elem *last) // first부터 last까지 delete
{
  ASSERT (is_interior (before) || is_tail (before));
  if (first == last)
    return;
...
list_shuffle (struct list *list) {
    ...
    for (int i = 0; i < 50; i++) {
        ...
        for (int i = 0; i < rand() % list_size(list); i++) a = list_next(a);
        for (int i = 0; i < rand() % list_size(list); i++) b = list_next(b);
        list_swap(a, b);
    }
}